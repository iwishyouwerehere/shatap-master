...
module.exports = function sendMessageEvent(socket) {
    return function eventHandler(request, response) {
        console.log('\x1b[34m' + '{core}@{sendMessageEvent}[request]');
        // check params
        if (!(request.params
        ...
        if (!socket.rooms[request.params['chatName']]) {
            console.log('\x1b[34m' + '{core}@{sendMessageEvent}[alert] request incoming from unsubscribed socket (unauthorized)');
